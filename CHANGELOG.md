...
### Changed
- **DISCO probing backs off the way `tailscaled` does** ([#46](https://github.com/Csontikka/esphome-tailscale/issues/46), the protocol direction). Three things in the DISCO manager had no bound. A CallMeMaybe was answered with a CallMeMaybe — the reference client never does that — so two microlink nodes without a WireGuard session bounced CallMeMaybe → ping burst → CallMeMaybe between each other indefinitely; that exchange is the ~9 packets/s storm behind #46 (51 CallMeMaybe in 150 s on the bench capture, each one costing an X25519 per endpoint on both sides). Every peer with a direct path was heartbeat-pinged every 3 s for ever, session or not. And a CallMeMaybe-triggered ping burst could be re-triggered as fast as the messages arrived. Now: no CallMeMaybe echo; the heartbeat runs only behind a live WireGuard session, a session-less peer is re-probed once a minute; CallMeMaybe bursts are spaced at least 2.5 s apart per peer. Direct paths are still found the way they were meant to be: each side probes the other's netmap endpoints and takes the address the PONG comes back from (on the bench, all six peers direct on both microlink nodes, with a WireGuard session between them). Idle peers **with** a live session keep their 3 s heartbeat: unlike `tailscaled`, this stack has one data-plane endpoint per peer and cannot send on two paths while it re-discovers, so the heartbeat is what keeps a quiet session on its direct path. Peers the netmap marks **offline** get neither the 15 s upgrade probe nor the 30 s DERP handshake retry any more (the reference client sends an idle peer nothing at all); on a router with 13 peers of which 8 were offline, those probes alone kept every 1 s manager tick above its 30 ms `SLOW` mark. The v0.5.7 work budget stays as the safety net; this change removes the load it was budgeting.
- **One X25519 per peer, not per DISCO packet.** Every DISCO ping, pong and CallMeMaybe — sent or received — recomputed the NaCl box shared secret from scratch, one X25519 each, ~16 ms on an ESP32-S3; with six session-ful peers' heartbeats that alone kept every 1 s manager tick above its 30 ms `SLOW` mark, and it is what made each packet of a storm expensive in the first place. The secret is now derived once per peer and cached (reference client: `discoInfo.sharedKey`), re-derived only when the peer's disco key changes. Inbound packets whose disco key belongs to no peer are dropped before any crypto, as the reference client does.

### Fixed
- **A dual-homed peer no longer forces a WireGuard handshake every 3 s.** Two things conspired. The best direct path followed *every* PONG, so a peer that answers from two addresses (its LAN address and its public NAT mapping are both in the netmap and both get our ping; a NAT that flips ports does the same) had its DISCO best flap; and an endpoint change on a live session with no recent data *forced a handshake*. On such a peer DISCO's best said one address while its WireGuard packets arrived from the other, wireguardif roamed the endpoint back on every reply, and the next heartbeat "switched" it again — one handshake per 3 s, indefinitely (measured on the router). Now the current best is kept while it answered within the last 6.5 s (the reference client's `trustUDPAddrDuration`), another address takes over only once the best has gone quiet, and an endpoint change never forces a handshake: WireGuard authenticates by key and roams by design, the existing keypair keeps working at the new address, and a peer that really lost the session is caught by the WireGuard timers and the DERP retry. The reference client never handshakes on an endpoint change either.
